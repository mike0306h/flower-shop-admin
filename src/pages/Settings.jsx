...
export default function Settings() {
  ...
  // === Printer state ===
  const [printer, setPrinter] = useState({ feieyun_user: '', feieyun_ukey: '', feieyun_sn: '', auto_print: false })
  ...
  const saveSmtp = async () => {
    ...
    try {
      ...
      setSmtpStatus({ ok: false, msg: e.response?.data?.detail || t('smtp_save_failed') || '保存失败' })
    } finally {
  ...
  const testSmtp = async () => {
    ...
    try {
      ...
      setSmtpStatus({ ok: false, msg: e.response?.data?.error || t('smtp_test_failed') || '测试失败' })
    } finally {
  ...
  const handleQrUpload = async (e) => {
    ...
    try {
      ...
      setShopInfo(s => ({ ...s, line_qr_image: res.url || '' }))
    } catch {
        ...
        <div className='flex overflow-x-auto'>
          {tabs.map(tab => (
            <button
              ...
              className={'flex-1 min-w-0 px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors ' + (activeTab === tab.id ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50')}
            >
          ...
          <div className='bg-white rounded-xl p-6 shadow-sm border border-slate-100'>
            ...
            <div className='space-y-4'>
              ...
              <div>
                ...
                {shopInfo.logo_url && (
                  <div className='mt-2'>
                    <img src={shopInfo.logo_url.startsWith('http') ? shopInfo.logo_url : '/api' + shopInfo.logo_url}
                      alt='Logo' className='w-16 h-16 object-contain border rounded-lg bg-white' />
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className='flex items-center gap-3'>
                  ...
                  {shopInfo.line_qr_image && <span className='text-xs text-green-600'>✓ {t('uploaded') || '已上传'}</span>}
                </div>
                {shopInfo.line_qr_image && (
                  <div className='mt-2'>
                    <img src={shopInfo.line_qr_image.startsWith('http') ? shopInfo.line_qr_image : '/api' + shopInfo.line_qr_image}
          ...
          <>
            <div className='bg-white rounded-xl p-6 shadow-sm border border-slate-100'>
              ...
              <div className='grid grid-cols-2 gap-3 mb-3'>
                ...
                <div>
                  <label className='block text-xs text-slate-500 mb-1'>{t('smtp_password')}</label>
            ...
            <div className='bg-white rounded-xl p-6 shadow-sm border border-slate-100'>
              ...
              <div className='grid grid-cols-2 gap-3 mb-3'>
                <div>
                  ...
                  <input value={printer.feieyun_user} onChange={e => setPrinter(p => ({ ...p, feieyun_user: e.target.value }))}
                    className='w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none' />
                </div>
                <div>
                  <label className='block text-xs text-slate-500 mb-1'>{t('feieyun_ukey')}</label>
                  <input type='password' value={printer.feieyun_ukey} onChange={e => setPrinter(p => ({ ...p, feieyun_ukey: e.target.value }))}
                    className='w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none' />
                </div>
                <div className='col-span-2'>
                  <label className='block text-xs text-slate-500 mb-1'>{t('feieyun_sn')}</label>
                  <input value={printer.feieyun_sn} onChange={e => setPrinter(p => ({ ...p, feieyun_sn: e.target.value }))}
                    className='w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none' />
                </div>
              </div>
              <div className='flex items-center gap-3 mb-4 p-3 bg-slate-50 rounded-lg'>
                <label className='relative inline-flex items-center cursor-pointer'>
                  <input type='checkbox' checked={printer.auto_print} onChange={e => setPrinter(p => ({ ...p, auto_print: e.target.checked }))} className='sr-only' />
